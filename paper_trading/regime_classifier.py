...
class RegimeClassifier:
    ...
    def classify(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        trend_score = (
            (df['kaufman_er'] * 1.3).clip(0, 1.0) * 0.45 +
            (df['adx'] / 45.0).clip(0, 1.0) * 0.55
        )
        ...
        df['regime_smooth_int'] = df['regime_int'].rolling(window=self.smoothing_window).apply(
            lambda x: pd.Series(x).mode().iloc[0] if not pd.Series(x).mode().empty else x[-1],
            raw=True
