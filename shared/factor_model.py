...
def factor_constrained_weights(
    ...
    """Compute weights with factor exposure constraints.
    ...
    Returns:
        {asset: weight} dict with factor constraints
    """
    if limits is None:
        limits = DEFAULT_FACTOR_LIMITS
    ...
    base_array = np.array([base[a] for a in assets])
    ...
    if result.success:
        ...
        return dict(zip(assets, [round(float(w), 6) for w in final]))
    else:
        return base
